...
print('Please input testnum:')
testnum=list(map(str,input().split()))[0]
...
for i in range(20):
    ...
    with open(filepath,'r',encoding='utf-8')as f:
        str1=f.read()
        str2= re.sub('[\x00-\x1F\x7F-\x9F]', '', str1)
        str2 = re.sub('\n', ' ', str2)
        #str2=re.sub('\.',' ',str2)
...
y_word=vectorizer.get_feature_names()
dictlist=[]
for i in range(len(tfidf_weight)):  # 打印每类文本的tf-idf词语权重，第一个for遍历所有文本，第二个for便利某一类文本下的词语权重
   # print(u"-------这里输出第", i, u"类文本的词语tf-idf权重------")
    worddict = dict()
    for j in range(len(y_word)):
           worddict[y_word[j]]=(tfidf_weight[i][j])/wordnum[i]
...
def wordcout(yy,y1,testnum,frequency=0):
    with open(f'D:\\YUAN JC\\#1#资讯管理学院\\2021-2022    大三图书馆学（下学期）\\科研学术训练\\2022.04.20\\wordcount{testnum}.csv','w',encoding='utf-8',newline='')as f:
        w=csv.writer(f)
...
def node_edge_BaseTfidf(rank,dictlist,testnum,edgethreshnum):   #不借助于词频的
    article_word=[]
    j = 0
    for dl in dictlist:
        j += 1
        ...
        everyarticle_word = []
        for l in L:
            everyarticle_word.append(l[0])
        article_word.append(everyarticle_word)
        #print(u"-------这里输出第", j, u"类文本tf-idf权重前20的词语------")
        #print(L)
    article_word
    node_str, edge_str = coocurance.build_matrix(article_word, True,edgethreshnum=edgethreshnum)
    with open(f'D:\\YUAN JC\\#1#资讯管理学院\\2021-2022    大三图书馆学（下学期）\\科研学术训练\\2022.04.20\\edge{testnum}.csv', 'w', encoding='utf-8') as csv:
        csv.write("Source,Target,Weight\r")
        csv.write(edge_str)
    with open(f'D:\\YUAN JC\\#1#资讯管理学院\\2021-2022    大三图书馆学（下学期）\\科研学术训练\\2022.04.20\\node{testnum}.csv', 'w', encoding='utf-8') as csv:
...
wordcout(yy,y1,testnum=testnum,frequency=3)
node_edge_BaseTfidf(dictlist=dictlist,rank=40,testnum=testnum,edgethreshnum=3)
