...
testnum=list(map(int,input().split()))[0]
...
def wordcout(yy,y1,testnum,frequency=0):
    with open(f'D:\\YUAN JC\\#1#资讯管理学院\\2021-2022    大三图书馆学（下学期）\\科研学术训练\\2022.04.20_(3)\\wordcount{testnum}.csv','w',encoding='utf-8',newline='')as f:
        w=csv.writer(f)
...
def node_edge_BasePara(yy,y1,y_array,nodethreshnum,testnum,edgethreshnum):   #不借助于词频的
    ...
    with open(f'D:\\YUAN JC\\#1#资讯管理学院\\2021-2022    大三图书馆学（下学期）\\科研学术训练\\2022.04.20_(3)\\edge{testnum}.csv', 'w', encoding='utf-8') as csv:
        csv.write("Source,Target,Weight\r")
        csv.write(edge_str)
    with open(f'D:\\YUAN JC\\#1#资讯管理学院\\2021-2022    大三图书馆学（下学期）\\科研学术训练\\2022.04.20_(3)\\node{testnum}.csv', 'w', encoding='utf-8') as csv:
...
wordcout(yy,y1,testnum=testnum,frequency=15)
node_edge_BasePara(yy,y1,y_array,nodethreshnum=15,testnum=testnum,edgethreshnum=15)
